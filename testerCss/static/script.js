...
arrowBtns.forEach((btn, i) => {
  ...
  btn.onclick = (e) => {
    ...
    if (i == 0) {
      ...
      currentCard++;
      currentCard = Math.min(6, currentCard); // Update to the total number of cards (including new ones)
    }
    moveCards(0.75);
  };
});
...
function moveCards(dur = 0) {
  gsap.timeline({ defaults: { duration: dur, ease: 'power3', stagger: { each: -0.03 * dir } } })
    ...
    .to('.arrow-btn-next', {
      autoAlpha: (currentCard == 6) ? 0 : 1 // Update to the total number of cards (including new ones)
    }, 0)
    .to('.card h4', {
